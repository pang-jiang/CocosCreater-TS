...
export class SocketDelegate implements ISocketDelegate {
    ...
    onSocketOpen() {
        ...
        EventManager.emit(SocketEvent.SOCKET_OPEN);
    }
    ...
    onSocketClosed(msg: string) {
        ...
        this._socket = null;
        EventManager.emit(SocketEvent.SOCKET_CLOSE);
    }
    ...
    onSocketMessage(data: string | ArrayBuffer) {
        ...
        EventManager.emit(msg.messageName, msg);
    }

    send(msg) {
        Log.log(LOG_TAG.SOCKET, 'send msg = ', msg);
