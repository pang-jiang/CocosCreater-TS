...
import UITips from "../../lobby/tips/UITips";
import UIConfirmDialog from "../../lobby/tips/UIConfirmDialog";
...
export default class UIHelp {
    ...
    public static ShowUI<T extends UIBase>(uiClass: UIClass<T>, callback?: Function, ...args: any[]) {
        UIManager.getInstance().openUI(uiClass, ViewZorder.UI, callback, null, ...args);
    }
    ...
    public static IsShowingUI<T extends UIBase>(uiClass: UIClass<T>) {
        return UIManager.getInstance().isShowing(uiClass);
    }

    public static ShowTips(message: string, ...param: any[]) {
        let tipUI = UIManager.getInstance().getUI(UITips) as UITips;
        if (!tipUI) {
            UIManager.getInstance().openUI(UITips, ViewZorder.Tips, (ui) => {
    ...
    public static ShowDialog(data: DialogParams) {
        UIManager.getInstance().openUI(UIConfirmDialog, ViewZorder.Dialog, null, null, data);
    }
}